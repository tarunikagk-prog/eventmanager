            ...
            {currentUser ? (
              <>
                ...
                {(currentUser.role === 'ADMIN' || currentUser.role === 'ORGANIZER') ? (
                  <Link to="/events/new" className="rounded-full px-3 py-2 transition hover:bg-white/5 hover:text-white">Create Event</Link>
                ) : null}
                <span className="rounded-full border border-indigo-400/30 bg-indigo-500/10 px-3 py-1 text-xs font-medium text-indigo-100">
                  {currentUser.name}
                </span>
                <button
